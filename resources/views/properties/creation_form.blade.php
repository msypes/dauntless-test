...
<form method="post" action="{{ ($editing)? url('properties/'.$property->id) : url('properties') }}"  enctype="multipart/form-data">
    {{csrf_field()}}
    ...
    <div class="input-wrapper">
        ...
        <textarea name="description" id="description" >{{($editing)? $property->description : '' }}</textarea>
    </div>
    <div class="input-wrapper">
    ...
    <div class="input-wrapper"><input type="submit"name="submit" value="{{($editing)? 'Edit' : 'Add' }} Property"></div>
</form>
</body>
</html>
